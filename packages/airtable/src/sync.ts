...
export async function runAirtableSync(
  ...
  for (const table of opts.tables ?? SYNC_TABLES) {
    ...
    if (creates.length > 0) {
      ...
      // d1Table comes from the static SYNC_TABLES config, never user input.
      const writeback = d1.prepare(`UPDATE ${table.d1Table} SET airtable_record_id = ? WHERE id = ?`);
      for (let i = 0; i < creates.length; i += WRITEBACK_BATCH) {
        await d1.batch(
          creates
            .slice(i, i + WRITEBACK_BATCH)
            .map((row, j) => writeback.bind(recordIds[i + j], row.id)),
        );
      }
    }
    if (updates.length > 0) {
    ...
    reports.push({ table: table.d1Table, created: creates.length, updated: updates.length });
  }
