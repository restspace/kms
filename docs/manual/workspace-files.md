...
speaker's name for uploads that aren't tied to a submission, like a headshot), Size, Versions
(how many versions exist), Uploaded, Event.
...
uploaded on a speaker's behalf), size, content type, and the latest upload date and version count,
all reflecting the current version even if the list row hasn't refreshed yet.
