...
| M1.8 | `do "go to Participant Information, click + Add Field, search the field library for 'Headshot', add it, and confirm a Headshot row appears at the bottom of the question list"` | Library pick — Headshot is a `contact`-scoped library field, so it is only offered on Participant Information — **FR-FORM-8** |
...
| M1.10 | `do "open Logic on the '<RUNID> Sponsor Code' question, tick 'Only show this question conditionally', add a condition referencing the earlier Biography question on the same step, save, and confirm the row now carries a Conditional chip"` | Conditional editor writes visibility — **FR-FORM-9**. The condition must reference an *earlier question on the same step*: the editor deliberately offers no cross-step references, so rules can never cycle |
...
| M3.6 | `do "on the 'Round 1 — Track leads' card select the round-robin strategy with 2 per submission, tick all three reviewers, press Assign and report the assignment note shown"` | `Round 1 — Track leads: N assignments across M submissions` — a plan created in M3.4 has no submissions (routing happens only via a form's routing rules at submission time), so assignment is exercised on the seeded plan — **FR-REV-9** |
...
| M3.8 | `do "set the 'Round 1 — Track leads' plan status to draft, then log in as frances.allen@example.com and confirm the draft plan's assignments do not appear in her review queue"` | Only `active` plans surface to reviewers — **FR-REV-9** |
...
| M3.11 | `do "as rosalind.franklin@example.com score every criterion on an open 'Round 1 — Track leads' assignment and press Save & Next; report the saved note and confirm the queue advances to the next unreviewed item"` | `Saved — your total X, submission mean Y` — **FR-REV-10** |
| M3.12 | `do "back as an admin, open that submission's detail tab in the workspace Submissions tab and report the reviews listed with their weighted totals"` | Every reviewer's review listed with its weighted total — **FR-REV-12** |
