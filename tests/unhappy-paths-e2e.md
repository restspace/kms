...
| M1.9a | `do "reopen Session Submission Form #2 in the builder after a full page reload and report the value shown in the Close Date field"` | Reads back `2026-08-01 00:00` — the same event-local wall clock that was typed, since a datetime typed in the admin is event-timezone, not viewer-timezone — **NFR-12**, **FR-FORM-13** |
